...
RETURNING user_id, email, token_version, created_at, last_login_at;

-- name: get_user_by_id
-- Fetch public account fields for `/v1/users/me` and other user lookups.
-- The password hash is intentionally not selected here.
SELECT user_id, email, token_version, created_at, last_login_at
FROM users
...
SELECT user_id, email, hashed_password, token_version, created_at, last_login_at
FROM users
WHERE email = %(email)s;
...
-- name: increment_user_token_version
-- Revoke previously issued user access tokens for logout and account security.
UPDATE users
